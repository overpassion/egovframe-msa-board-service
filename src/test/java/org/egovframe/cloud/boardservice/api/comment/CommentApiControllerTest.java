...
class CommentApiControllerTest {
    ...
    void 게시글의_댓글_목록_조회() {
        ...
        final String url = URL + "/" + posts.getPostsId().getBoardNo() + "/" + posts.getPostsId().getPostsNo();

        // when
        ResponseEntity<List<CommentListResponseDto>> responseEntity = restTemplate.exchange(
                url,
                HttpMethod.GET,
                null,
        ...
        List<CommentListResponseDto> list = responseEntity.getBody();
        assertThat(list).isNotNull();
        assertThat(list.size()).isEqualTo(GIVEN_DATA_COUNT);
        assertThat(list)
                .isNotEmpty()
                .has(new Condition<>(l -> (INSERT_COMMENT_CONTENT + "1").equals(l.get(0).getCommentContent()), "CommentApiControllerTest.게시글의_댓글_목록_조회 contains " + INSERT_COMMENT_CONTENT + "1"))
                .has(new Condition<>(l -> (INSERT_COMMENT_CONTENT + "2").equals(l.get(1).getCommentContent()), "CommentApiControllerTest.게시글의_댓글_목록_조회 contains " + INSERT_COMMENT_CONTENT + "2"));
    }
    ...
    private void insertComments() {
        ...
        comments = commentRepository.saveAll(list);
    }
